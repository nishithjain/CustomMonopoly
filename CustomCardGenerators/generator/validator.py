...
def _validate_templates(result: ValidationResult) -> None:
    for name in REQUIRED_TEMPLATES:
        ...
        if name == "property_card.html" and "const CARD_DATA" not in text:
            _error(result, "templates", f"Template: {name}", "CARD_DATA", "Could not find CARD_DATA block.")
        if name == "board.html":
            if "const boardSpaces" not in text or "const GO_DATA" not in text:
                _error(result, "templates", f"Template: {name}", "boardSpaces", "Could not find boardSpaces/GO_DATA.")
        if name == "event_card.html":
            for token in (
                "@@PAGE_TITLE@@",
                "@@EVENT_NAME@@",
                "@@EVENT_SUBTITLE@@",
...
def _validate_assets(result: ValidationResult) -> None:
    for name in REQUIRED_BOARD_ASSETS:
        path = ASSETS_COMMON_DIR / name
        if not path.is_file():
            _error(result, "assets", f"Asset: {name}", name, f"Missing common board asset at {path}")
